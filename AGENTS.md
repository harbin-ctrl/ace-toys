...
- On Windows each toy has a per-user installer: `make install` at the
  repository root, in an MSYS2 CLANGARM64 shell, builds and runs them. See
  `win-packaging`.
...
- Format C/C++ with AROS's AStyle profile (`astyle --style=kr --indent=spaces=4 --attach-closing-while --pad-oper --unpad-paren --align-pointer=name --max-code-length=120 --lineend=linux`). Keep empty lines between logical blocks of code.
